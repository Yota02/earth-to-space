<script>
export default {
  data() {
    return {
      ws: null,
      connectionStatus: 'connecting',
      gameData: {
        argent: 0,
        objectsAchetables: []
      }
    }
  },

  computed: {
    fuelObjects() {
      return this.gameData.objectsAchetables.filter(obj => obj.type === 'carburant') || [];
    }
  },

  methods: {
    initWebSocket() {
      this.ws = new WebSocket('ws://localhost:3232');

      this.ws.onopen = () => {
        this.connectionStatus = 'connected';
      };

      this.ws.onclose = () => {
        this.connectionStatus = 'error';
      };

      this.ws.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data);
          this.gameData = data;
        } catch (error) {
          console.error('Erreur lors du parsing du message:', error);
        }
      };
    },

    calculateFillPercentage(fuel) {
      if (!fuel.quantiteStock || !fuel.capaciteMax) return 0;
      return Math.min((fuel.quantiteStock / fuel.capaciteMax) * 100, 100);
    },

    getFuelColor(percentage) {
      if (percentage > 66) return '#22c55e';  // Vert
      if (percentage > 33) return '#eab308';  // Jaune
      return '#ef4444';  // Rouge
    },

    handlePurchase(fuelName) {
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        this.ws.send(JSON.stringify({
          action: 'buyObject',
          name: fuelName
        }));
      }
    },

    handleSell(fuelName) {
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        this.ws.send(JSON.stringify({
          action: 'sellObject',
          name: fuelName
        }));
      }
    }
  },

  mounted() {
    this.initWebSocket();
  },

  beforeUnmount() {
    if (this.ws) {
      this.ws.close();
    }
  }
}
</script>

<template>
  <div class="p-6 max-w-7xl mx-auto">
    <!-- État d'erreur -->
    <div v-if="connectionStatus === 'error'" class="p-6 text-center text-red-600">
      Erreur de connexion au serveur. Veuillez rafraîchir la page.
    </div>

    <!-- État de chargement -->
    <div v-else-if="connectionStatus === 'connecting'" class="p-6 text-center">
      Connexion au serveur...
    </div>

    <!-- Contenu principal -->
    <template v-else>
      <div class="fuel-container">
        <div
          v-for="fuel in fuelObjects"
          :key="fuel.nom"
          class="fuel-item"
        >
          <h4 class="font-semibold text-lg mb-2">{{ fuel.nom }}</h4>

          <!-- Réservoir -->
          <div class="reservoir">
            <div class="tank">
              <div
                class="fuel-level"
                :style="{
                  height: calculateFillPercentage(fuel) + '%',
                  backgroundColor: getFuelColor(calculateFillPercentage(fuel))
                }"
              ></div>
            </div>
            <div class="info mt-3">
              <p class="text-sm text-gray-600">
                Quantité: {{ fuel.quantiteStock?.toFixed(2) || 0 }} L
              </p>
              <p class="text-sm text-gray-600">
                Capacité: {{ fuel.capaciteMax?.toFixed(2) || 0 }} L
              </p>
              <p class="text-sm text-gray-600">Prix: {{ fuel.prix }} crédits/L</p>
            </div>
          </div>

          <!-- Boutons -->
          <div class="flex gap-2 mt-3">
            <button
              @click="handlePurchase(fuel.nom)"
              class="flex-1 px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              :disabled="gameData.argent < fuel.prix"
            >
              Acheter
            </button>
            <button
              @click="handleSell(fuel.nom)"
              class="flex-1 px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded-md transition-colors"
            >
              Vendre
            </button>
          </div>
        </div>
      </div>
    </template>
  </div>
</template>

<style scoped>
.fuel-container {
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  gap: 2rem;
  justify-content: center;
  align-items: flex-start;
}

.fuel-item {
  flex: 0 1 auto;
  min-width: 200px;
  max-width: 250px;
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 1rem;
  border: 1px solid #e5e7eb;
  border-radius: 0.5rem;
  box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);
}

.reservoir {
  display: flex;
  flex-direction: column;
  align-items: center;
  margin-bottom: 1rem;
}

.tank {
  position: relative;
  width: 60px;
  height: 200px;
  border: 2px solid #444;
  background-color: #f3f4f6;
  border-radius: 6px;
  overflow: hidden;
}

.fuel-level {
  position: absolute;
  bottom: 0;
  width: 100%;
  transition: height 0.5s, background-color 0.5s;
}

.info {
  text-align: center;
  margin-top: 0.5rem;
}
</style>